          ...
          <div class="card course-box"> 
            <div class="card-body"> 
              <div class="course-widget"> 
                ...
                <div> 
                  <h4 class="mb-0">{{ $pendingSession }} </h4><span class="f-light">Pending Approval Sessions </span><a class="btn btn-light f-light" href="{{ route('program-parent')}}">View program<span class="ms-2"> 
                      <svg class="fill-icon f-light">
                        <use href="{{ asset('assets/svg/icon-sprite.svg#arrowright') }}"></use>
                      </svg></span></a>
                </div>
        ...
        <div class="card schedule-card">
          <div class="card-header card-no-border">
            <div class="header-top">
              <h5 class="m-0">Upcoming Sessions</h5>
              
            </div>
          </div>
          <div class="card-body pt-0"> 
            <ul class="schedule-list"> 
              @forelse ($childSchedule as $schedule)
              <li class="primary"><img src="{{ asset('assets/images/dashboard/user/4.jpg') }}" alt="profile">
                <div> 
                  <h6 class="mb-1">Session {{$schedule->session}} </h6>
                  <ul>
                    <li class="f-light">
                      <svg class="fill-icon f-light">
                        <use href="{{ asset('assets/svg/icon-sprite.svg#bag') }}"></use>
                      </svg><span>{{$schedule->date}}</span>
                    </li>
                    <li class="f-light">
                      <svg class="fill-icon f-success">
                        <use href="{{ asset('assets/svg/icon-sprite.svg#clock') }}"></use>
                      </svg><span>{{$schedule->time}} </span>
                    </li>
                  </ul>
                </div>
              </li>
              @empty
              <span>There's no upcoming session for this month</span>
                  @endforelse
              </li>
            </ul>
          </div>
...
<script>
  ...
  var sessionTotal = {{ $sessionQuantity }}
</script>
